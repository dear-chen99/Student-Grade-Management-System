...
class TestLoginValidation(unittest.TestCase):
    ...
    def test_invalid_username(self):
        """测试无效用户名"""
        is_valid = ("wrong" == self.valid_username
                    and "123456" == self.valid_password)
        self.assertFalse(is_valid)

    def test_invalid_password(self):
        """测试无效密码"""
        is_valid = ("admin" == self.valid_username
                    and "wrong" == self.valid_password)
        self.assertFalse(is_valid)

    def test_empty_username(self):
        """测试空用户名"""
        is_valid = ("" == self.valid_username
                    and "123456" == self.valid_password)
        self.assertFalse(is_valid)
    ...
    def test_whitespace_credentials(self):
        ...
        is_valid = (username == self.valid_username
                    and password == self.valid_password)
...
class TestLoginCredentials(unittest.TestCase):
    ...
    def test_correct_credentials_match(self):
        ...
        result = (creds["username"] == LOGIN_CONFIG["default_username"]
    ...
    def test_partial_credentials(self):
        """测试部分凭据"""
        for pw in ["", " ", "\t"]:
            result = ("admin" == LOGIN_CONFIG["default_username"]
...
class TestLoginWindowModule(unittest.TestCase):
    ...
    def test_login_window_attributes(self):
        """测试 LoginWindow 类属性"""
        expected = {"__init__", "run", "_login_action",
                    "_prepare_and_show", "_preload_background", "_show_window"}
        actual = {name for name in dir(LoginWindow)
    ...
    def test_mock_login_validation(self):
        ...
        def mock_validate(username, password):
            return username == LOGIN_CONFIG["default_username"] \
...
class TestLoginValidationEdgeCases(unittest.TestCase):
    ...
    def test_validate_empty_both(self):
        ...
        def validate(u, p):
            if not u or not p:
                return False
            return u == LOGIN_CONFIG["default_username"] \
    ...
    def test_strip_validation(self):
        ...
        def validate_stripped(u, p):
            return u.strip() == LOGIN_CONFIG["default_username"] \
